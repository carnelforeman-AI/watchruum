 ...
 * filter string (e.g. `.or("display_name.ilike.%<q>%")`). Strips characters
 * that carry meaning in the PostgREST filter grammar — commas, parentheses,
 * dots, wildcards, quotes, backslashes and control chars — so a crafted query
 * can't break out of the intended condition. Also caps length.
 */
...
export const getAdminSnapshot = cache(async (): Promise<AdminSnapshot> => {
  ...
  const { data: reports } = await supabase
    .from("reports")
    .select("id, target_type, target_id, reason, status, created_at, reporter:profiles!reports_reporter_id_profiles_fkey(display_name)")
